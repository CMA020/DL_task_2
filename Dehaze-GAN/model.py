...
class GAN():
    ...
    def build_model(self):
        ...
        with tf.name_scope('DiscriminatorLoss'):
            self.D_loss = tf.reduce_mean(-(tf.math.log(self.predict_real + self.EPS) + tf.math.log(1 - self.predict_fake + self.EPS)))

        with tf.name_scope('GeneratorLoss'):
            self.gan_loss = tf.reduce_mean(tf.negative(self.predict_fake + self.EPS))
            ...
            self.G_loss = tf.reduce_mean(self.G_loss, axis=[0, 1])
            # self.gan_loss = tf.reduce_mean(tf.negative(self.predict_fake + self.EPS))
            # self.l1_loss = tf.reduce_mean(tf.abs(self.RealB - self.FakeB))
            # self.vgg_loss = (1e-5) * tf.losses.mean_squared_error(self.RealB_VGG, self.FakeB_VGG)
            # self.G_loss = self.gan_loss + self.l1_loss + self.vgg_loss
            # self.G_loss_mean = tf.reduce_mean(self.G_loss, axis=[0, 1])

            #G_loss_sum = tf.summary.scalar('Generator Loss', self.G_loss_mean)
    ...
    def train(self):
        ...
        if self.custom_data:
            ...
            val_data = data[: int(total_image_count*self.val_fraction)]
            ...
            for file in train_data:
                img_path_A = os.path.join(self.A_dir, file)
                img_path_B = os.path.join(self.B_dir, file)
                img_A = cv2.imread(img_path_A)
                img_B = cv2.imread(img_path_B)
                if img_A is not None and img_B is not None:
                    self.A_train[train_index] = img_A.astype(np.float32)
                    self.B_train[train_index] = img_B.astype(np.float32)
                    train_index += 1
            ...
            for file in val_data:
                ...
                img_B = cv2.imread(img_path_B)
                if img_A is not None and img_B is not None:
                    self.A_val[val_index] = img_A.astype(np.float32)
                    self.B_val[val_index] = img_B.astype(np.float32)
                    val_index += 1
        ...
        with tf.compat.v1.Session() as self.sess:
            ...
            total_parameter_count = tf.reduce_sum([tf.reduce_prod(tf.shape(v)) for v in tf.compat.v1.trainable_variables()])
            G_parameter_count = tf.reduce_sum([tf.reduce_prod(tf.shape(v)) for v in tf.compat.v1.trainable_variables() if v.name.startswith("Generator")])
            D_parameter_count = tf.reduce_sum([tf.reduce_prod(tf.shape(v)) for v in tf.compat.v1.trainable_variables() if v.name.startswith("Discriminator")])
            ...
            for i in range(start, self.epochs):
                ...
                for j in range(batches):
                    ...
                    #     tf.summary.scalar('D_summary', D_summary, step=step)
                    #     self.writer.flush()
